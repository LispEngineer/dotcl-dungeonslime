...
| July 4, 2026 | [title-scene.lisp](title-scene.lisp) | Modified | Updated to load and use the larger fonts/04B_30_5x spritefont for title rendering, and standard fonts/04B_30 for prompt rendering. Redesigned layout to split "DUNGEON SLIME" into two centered white lines, and removed the MonoGame logo. |
...
- **Title Screen Redesign**: Redesigned the title screen (`title-scene.lisp`) layout to match the original tutorial game:
  - Removed the `logo-texture` slot and code to load and draw the MonoGame logo (which should not be shown).
