...
This document describes the implementation of version 10 of the
...
`Rectangle.Contains` were skipped entirely, requiring hand-written
wrappers in `mg-classes.lisp`.
...
- **`cspackages-test.lisp`** — New file with 30+ integration test
  assertions covering Vector2, Rectangle, Point, and TimeSpan packages.
  Tests verify correct results, passthrough dispatch, typed overloads,
  property accessors, constants, and error signaling on bad type args.

- **`mg-classes.lisp`** — Removed hand-written wrappers:
  `v2-distance-squared`, `v2-reflect`, `rect-intersects`,
  `rect-contains-p`. Reimplemented `v2-normalize` to use generated
  `microsoft-xna-framework-vector2:length` and
  `microsoft-xna-framework-vector2:/` instead of raw
  `dotnet:invoke`/`dotnet:static`.

- **`game-1.lisp`**, **`collision.lisp`**, **`collision-test.lisp`** —
  Updated callers to use generated cspackage functions instead of
  removed `mg-classes` wrappers.

- **`packages.lisp`** — Added pre-declaration for
  `:microsoft-xna-framework-point`. Added `:point` and `:time-span`
  local-nicknames to `:dungeon-slime` package.

- **`dungeon-slime.asd`** — Added `cspackages-test` component with
  dependencies on `utils`, `mg-classes`, and all cspackages components.

- **`test-harness.lisp`** — Added `(run-cspackages-tests)` call.
