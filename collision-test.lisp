...
(defun run-collision-tests ()
  ...
  (let* ((v (vector2 3.0e0 4.0e0))
         (n (v2-normalize v))
         (len (dotnet:invoke n "Length")))
    ;; Use approximate comparison for floating point
    (assert (< (abs (- len 1.0e0)) 0.0001e0))
    (format *error-output* "  [PASS] v2-normalize (3,4) has length ~A (expected ~1)~%" len 1.0e0))

  ;; Normalize zero vector -> zero vector
  (let* ((v (vector2 0.0e0 0.0e0))
         (n (v2-normalize v)))
